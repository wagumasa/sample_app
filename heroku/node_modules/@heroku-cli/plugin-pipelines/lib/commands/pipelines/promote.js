...
function assertNotPromotingToSelf(source, target) {
    chai_1.assert.notStrictEqual(source, target, `Cannot promote from an app to itself: ${target}. Specify a different target app.`);
}
function findAppInPipeline(apps, target) {
    const found = apps.find(app => (app.name === target) || (app.id === target));
    chai_1.assert(found, `Cannot find app ${color_1.default.app(target)}`);
    return found;
}
